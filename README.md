...
![\Large \min S(A,B)=1/2\|X-XBA^T\|_F^2+\lambda\|B\|_2^2+\lambda_1\sum_{j=1}^{k}p_1(\beta_j)+\lambda_2\sum_{j=1}^{k}p_2(\beta_j)](https://latex.codecogs.com/svg.latex?\Large&space;\min S(A,B)=1/2\|X-XBA^T\|_F^2+\lambda\|B\|_2^2+\lambda_1\sum_{j=1}^{k}p_1(\beta_j)+\lambda_2\sum_{j=1}^{k}p_2(\beta_j)) 
